...
//강좌 아이디
$courseid    = optional_param('merger_course_id', 0, PARAM_INT);
...
$params = array('dept' => '%' .$dept. '%' ,'juya' => '%' .$juya. '%','hyear' => '%' .$hyear. '%',
    'searchtxt1' => '%' . $searchtext . '%', 'searchtxt2' => '%' . $searchtext . '%',
    'searchtxt3' => '%' . $searchtext . '%','contextlevel'=>CONTEXT_COURSE,
    'instanceid'=>$courseid, 'roleid'=>5, 'roleid2'=>9);
...
<div class="popup_content" id="course_stu">
    <form id="frm_course_stu" class="search_area" onsubmit="course_prof_search(); return false;" method="POST">
        ...
        <input type="hidden" name="merger_course_id" value="<?php echo $courseid; ?>" />
        <b>주야구분 : </b> 
        ...
        <select title="학과" name="dept" class="w_160">
            ...
            <?php
                foreach ($dept_lists as $dept_list){
                    ...
                    if($dept_list->ohakkwa == null || $dept_list->ohakkwa == '' ){
                        continue;
                    }else{
...
<script type="text/javascript">
    function course_prof_search() {
        ...
        var merger_course_id = $( "#frm_course_stu input[name=merger_course_id]" ).val();
        ...
        $.ajax({
            url: '<?php echo $CFG->wwwroot.'/siteadmin/manage/course_merger_list.php'; ?>',
            method: 'POST', 
            data: { 
                'value': searchstring,
                'merger_course_id':merger_course_id,
                'juya' : juya,
                'hyear' : hyear,
                'dept' : dept
            },
