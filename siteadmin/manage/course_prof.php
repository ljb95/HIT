        ...
        <table cellpadding="0" cellspacing="0">
            ...
            if($usercount > 0) {
                ...
                foreach($users as $user) {
                    ...
                    echo '<td><input type="button" value="'.get_string('add','local_lmsdata').'" class="orange_btn" onclick="course_prof_select(\''.$user->id.'\', \''.addslashes(fullname($user)).'\');"/></td>';
                    echo '</tr>';
...
<script type="text/javascript">
    ...
    function course_prof_select(id, name) {
        $( "input[name=prof_userid]" ).attr("value",id);
        $( "input[name=prof_name]" ).attr("value",name);
        if($( "input[name=prof_username]" )){
            $( "input[name=prof_username]" ).attr("value",name);
        }
