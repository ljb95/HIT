...
if (isguestuser()) {
    $SESSION->wantsurl = (string)new moodle_url('/siteadmin/stats/contact_stats_week.php');
...
$sql_select  = "SELECT  CONCAT(FROM_UNIXTIME(lsl.log_date,'%u'),mobile) as dt, FROM_UNIXTIME(lsl.log_date,'%u') as week, COUNT(*) AS count,mobile , lsl.log_date ";
...
$sql_groupby = " GROUP BY FROM_UNIXTIME(lsl.log_date,'%u'),mobile";

$contact_stats = $DB->get_records_sql($sql_select.$sql_from.$sql_where.$sql_groupby, $param);

foreach ($contact_stats as $contact) {
    $weeknum = get_weeknum($contact->log_date);
    if(isset($datas[number_format($weeknum)][$contact->mobile])){
        $datas[number_format($weeknum)][$contact->mobile] = $contact->count;
    } else {
        $datas[number_format($weeknum)][$contact->mobile] += $contact->count;
    }
    $datas[number_format($weeknum)]['logdate'] = $contact->log_date;
}
for ($i = 1; $i <= 6; $i++) {
            $js_string .= '{
                        "week": "' . $i . '주차",';
...
<div id="contents">
    ...
    <div id="content">
        ...
        <div class="siteadmin_tabs">
            ...
            <a href="contact_stats_week.php"><p class="black_btn black_btn_selected"><?php echo get_string('stats_week', 'local_lmsdata'); ?></p></a>
        ...
        <div class="page_navbar"><a href="./contact_stats_week.php"><?php echo get_string('stats_management', 'local_lmsdata'); ?></a> > <?php echo get_string('stats_loginstats', 'local_lmsdata'); ?></div>
        
        
        <form name="" id="course_search" class="search_area" action="contact_stats_week.php" method="get">
        ...
        <?php
            $url = $CFG->wwwroot.'/siteadmin/stats/contact_stats_week.php?'; 
