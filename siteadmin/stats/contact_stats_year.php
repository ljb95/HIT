...
if (isguestuser()) {
    $SESSION->wantsurl = (string)new moodle_url('/siteadmin/stats/contact_stats_year.php');
...
$sql_select  = "SELECT CONCAT(FROM_UNIXTIME(lsl.log_date,'%m'),mobile) as dt,FROM_UNIXTIME(lsl.log_date,'%m') as month, COUNT(*) AS count ,mobile, lsl.log_date ";
...
$param['isloggedin'] = 'logged';
    ...
    $date_end =  strtotime($year.'-12-31 23:59:59');
...
$sql_groupby = " GROUP BY FROM_UNIXTIME(lsl.log_date,'%m'),mobile";

$contact_stats = $DB->get_records_sql($sql_select.$sql_from.$sql_where.$sql_groupby, $param);

$datas = array();
foreach ($contact_stats as $contact) {
    $datas[number_format($contact->month)][$contact->mobile] = $contact->count;
    $datas[number_format($contact->month)]['logdate'] = $contact->log_date;
}
...
<div id="contents">
    ...
    <div id="content">
        ...
        <div class="siteadmin_tabs">
            ...
            <a href="contact_stats_year.php"><p class="black_btn black_btn_selected"><?php echo get_string('stats_yearly', 'local_lmsdata'); ?></p></a>
        ...
        <form name="" id="course_search" class="search_area" action="contact_stats_year.php" method="get">
        ...
        <?php
            $url = $CFG->wwwroot.'/siteadmin/stats/contact_stats_year.php?'; 
