...
$returnurl = $CFG->wwwroot.'/local/okteam/index.php?id='.$courseid;
$rooturl   = $CFG->wwwroot.'/local/okteam/overview.php?id='.$courseid;

if (!$course = $DB->get_record('course', array('id'=>$courseid))) {
    print_error('invalidcourse');
}

$url = new moodle_url('/local/okteam/overview.php', array('id'=>$courseid));
if ($groupid !== 0) {
...
/// Print overview
echo $OUTPUT->heading(format_string($course->fullname, true, array('context' => $context)) .' '.$stroverview, 3);
