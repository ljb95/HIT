        ...
        if ($totalcount > 0) {
            ...
            foreach ($courses as $course) {
                ...
                if ($course->enroled == 0) {
                    ...
                    $regedbutton = '<button onclick="apply_enrol(' . $course->id . ');" class="btn_small">' . get_string('apply', 'local_okregular') . '</button>';
                } else {
                    if ($course->userenrolstatus == 0) {
                        $regedstatustext = get_string('course:registered', 'local_okregular');
                        $regedbutton = '<button class="blue_btn_small">'.get_string('course:registered','local_okregular').'</button>';
                    ...
                    <td scope="col">
                        <?php
                            if ($course->term == 1 || $course->term == 2) {
                                $term_text = $course->term . get_string('stats_terms', 'local_lmsdata');
                            } else if ($course->term == 3 || $course->term == 4) {
                                $term_text = str_replace(array(3, 4), array(get_string('summer', 'local_okregular')), $course->term);
                            } else {
                                $term_text = '-';
                            }
                            echo ($course->term == 0)? get_string('term:all','local_okregular'):$term_text; 
                        ?>
                    </td>
