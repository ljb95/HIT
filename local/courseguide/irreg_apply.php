...
$PAGE->set_url('/local/courseguide/irreg_apply.php');

$pluginnameplural = get_string("pluginnameplural", "local_courseguide");
$strplural = get_string("course:irregulal", "local_courseguide");
$PAGE->navbar->add($pluginnameplural);
...
//tab
$row = array();
$row[] = new tabobject('1', "$CFG->wwwroot/local/courseguide/irreg_apply.php?type=1", get_string('ongoing', 'local_okirregular'));
$row[] = new tabobject('2', "$CFG->wwwroot/local/courseguide/irreg_apply.php?type=2", get_string('finish', 'local_okirregular'));
$rows[] = $row;
...
<table class="generaltable" id="table_courses">
    ...
    <thead>
        <tr>
            ...
            <th scope="row" width="10%"><?php echo get_string('courseimg', 'local_courseguide'); ?></th>
    ...
    <tbody>
        ...
        if ($totalcount > 0) {
            ...
            foreach ($courses as $course) {
                $moodle_course = new course_in_list($course);
                ...
                <tr>
                    ...
                    <td class="courseimg">
                        <?php
                         if($moodle_course->get_course_overviewfiles()){
                        foreach ($moodle_course->get_course_overviewfiles() as $file) {
                            ...
                            if ($isimage) {
                                echo html_writer::tag('div',
                                        html_writer::empty_tag('img', array('src' => $url,'class'=>'course_img','onclick'=>'window.open("'.$url.'","","")','width'=>'45px', 'alt' => $moodle_course->fullname, 'title' => $moodle_course->fullname)),
                                        array('class' => 'courseimage'));
                            } 
                        }
                    }else{
                        $url = '/theme/remui/pix/no_img105.jpg';
                        echo html_writer::tag('div',
                                 html_writer::empty_tag('img', array('src' => $url,'class'=>'empty_img','width'=>'45px', 'alt' => $moodle_course->fullname, 'title' => $moodle_course->fullname)),
                                array('class' => 'courseimage'));            
                    }
                        ?>
                    </td>
                    <td scope="col" class="title">
                    <a href="courseinfo_irr.php?id=<?php echo $course->id?>"><?php echo $course->course_name; ?></a>
                    </td>
                    <td scope="col">
                        <?php
