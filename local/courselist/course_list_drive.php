...
$sql_select  = "SELECT lcd.id, lcd.invisible, lcd.timecreated, lcd.restore_flag
     ,mc.fullname AS subjectname
     , sco.fullname AS standardname
     , lc.subject_id, lc.isnonformal
     , dus.firstname AS exefirstname, dus.lastname AS exelastname
     , mu.firstname AS proffirstname,  mu.lastname AS proflastname ";
...
$sql_where   = " WHERE ((lc.year = :year 
  AND lc.term = :term ) or (lc.year= 9999)) AND lcd.type = :type and ctx.contextlevel =:contextlevel and ra.userid =:userid  ";
...
$params = array(
    ...
    'userid'=>$USER->id,
    'type'=>LMSDATA_CLASS_MERGE_EXECUTE
);
...
$PAGE->set_url('/local/courselist/course_list_drive.php');
...
$strplural = get_string("course:classes_drive_log", "local_courselist");
...
?>
        <table class="generaltable" id="course_divide">
            <thead>
            <tr>
                ...
                <th class="col-7"><?php echo get_string('course:visible', 'local_courselist'); ?></th>
                <th class="col-8"><?php echo get_string('regular:short', 'local_courselist').'/'.get_string('irregular:short', 'local_courselist'); ?></th>
                <th class="col-9"><?php echo get_string('date:execute', 'local_courselist'); ?></th>
                <th class="col-10"><?php echo get_string('course:restore', 'local_courselist'); ?></th>
            </tr>
            </thead>
            <tbody>
            <?php
            ...
            <tr>
                ...
                <td class="col-4"><?php echo $course->exefirstname.$course->exelastname; ?></td>
                <td class="col-7"><?php echo $course->invisible == 1 ? get_string('visible', 'local_courselist') : get_string('invisible', 'local_courselist'); ?></td>
                <td class="col-8"><?php echo $course->isnonformal == 1 ? get_string('irregular:short', 'local_courselist') : get_string('regular:short', 'local_courselist'); ?></td>
                <td class="col-9"><?php echo date("Y-m-d", $course->timecreated); ?></td>
                <td class="col-10">
                    <?php
                        if(empty($course->restore_flag)) {
                            echo '<input type="button" class="red_btn" value="'.get_string('course:restore', 'local_courselist').'" onclick="restore_course_execute('.$course->id.')"/>';
                        } else {
                            echo get_string('success', 'local_courselist');
                        }
                    ?>
                </td>
            </tr>
...
function restore_course_execute(id){
    if(confirm('<?php echo get_string('course:restore_exe', 'local_courselist'); ?>')) {
        $.ajax({
            url : "<?php echo $CFG->wwwroot."/local/courselist/course_list_restore.execute.php";?>",
            type: "post",
            data : {
                data : id                            
            },
            ...
            success: function(data){
               document.location.href = "<?php echo $CFG->wwwroot."/local/courselist/course_list_drive.php"?>";
            },
            error:function(e){
