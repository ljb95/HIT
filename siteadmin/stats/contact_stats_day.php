...
if (isguestuser()) {
    $SESSION->wantsurl = (string) new moodle_url('/siteadmin/stats/contact_stats_day.php');
...
$sql_select = "SELECT CONCAT(FROM_UNIXTIME(lsl.log_date,'%H'),mobile) as dt, FROM_UNIXTIME(lsl.log_date,'%H') as hour,mobile, COUNT(*) AS count , lsl.log_date ";
...
$sql_groupby = " GROUP BY FROM_UNIXTIME(lsl.log_date,'%H') , mobile";
...
foreach ($contact_stats as $contact) {
    $datas[number_format($contact->hour)][$contact->mobile] = $contact->count;
    $datas[number_format($contact->hour)]['logdate'] = $contact->log_date;
}
...
<?php include_once ('../inc/sidebar_stats.php'); ?>
    <div id="content">
        ...
        <div class="siteadmin_tabs">
            <a href="contact_stats_day.php"><p class="black_btn black_btn_selected"><?php echo get_string('stats_daily', 'local_lmsdata'); ?></p></a>
        ...
        <div class="page_navbar"><a href="./contact_stats_day.php"><?php echo get_string('stats_management', 'local_lmsdata'); ?></a> > <?php echo get_string('stats_loginstats', 'local_lmsdata'); ?></div>

        

        <form name="" id="course_search" class="search_area" action="contact_stats_day.php" method="get">
            ...
            <select name="day" class="w_80">
                <option value="0"> - <?php echo get_string('contents_now', 'local_lmsdata'); ?> -</option>
                <?php
                $days = lmsdata_get_days($day);
                echo $days;
                ?>
            </select>
        ...
        <?php
            $url = $CFG->wwwroot.'/siteadmin/stats/contact_stats_day.php?'; 
        ...
        <div class="graph_toggle toggle <?php if($view_type == 'graph'){ echo 'selected'; } ?>" onclick="location.href='<?php echo $url.'graph'; ?>'"></div>
         ...
         <table>
            ...
            ?>
            <td style='width:10%;'>
                <input type="button" title="<?php echo get_string('stats_view', 'local_lmsdata'); ?>" alt="<?php echo get_string('stats_view', 'local_lmsdata'); ?>"
                       id="view" class="blue_btn" value="<?php echo get_string('stats_view', 'local_lmsdata'); ?>"
                       onclick="contact_log(<?php echo strtotime($year.'-'.$mon.'-'.$day.' '.str_pad($i,2,0,STR_PAD_LEFT).':00:00');?>)"></td>
            <?php
            echo '</tr>';
            }  ?>
         </table>
