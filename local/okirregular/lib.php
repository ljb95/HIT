...
define('COURSE_TYPE', '1'); //비교과과정 구분값
...
function local_okirregular_get_all_courses($categoryid, $param, $page=1, $perpage=10, $sort=" c.fullname ASC") {
    ...
    if($page == 0){
        ...
        $courses = $DB->get_records_sql($sql.$sort, $params, local_okirregular_offset($page, $perpage), $perpage);
    }
...
function local_okirregular_category($categoryid){
   global $DB;
...
function local_okirregular_offset($page, $limit) {
    return $limit * ($page-1);
}


function local_okirregular_print_paging_navbar($totalcount,$page_name, $page, $perpage, $baseurl, $params = null, $maxdisplay = 10) {
    $pagelinks = array();
...
function local_okirregular_print_paging_navbar_script($totalcount, $page, $perpage, $baseurl, $maxdisplay = 10) {
    $pagelinks = array();
...
function local_okirregular_set_assign_user($courseid, $userid, $rolename){
    global $CFG, $PAGE, $DB;
...
function local_okirregular_set_unassign_user($courseid, $userid) {
