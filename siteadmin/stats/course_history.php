...
$sql_select = "SELECT mc.id, mc.fullname, mc.shortname , mc.format as courseformat 
 ...
 ,(SELECT COUNT(*) FROM {role_assignments} ra JOIN {role} ro ON ra.roleid = ro.id JOIN {context} ctx ON ra.contextid = ctx.id
        JOIN {course} co ON ctx.instanceid = co.id AND contextlevel = :contextlevel1 WHERE co.id = mc.id AND ro.id = 5) as student 
 ,(SELECT COUNT(*) FROM {role_assignments} ra JOIN {role} ro ON ra.roleid = ro.id JOIN {context} ctx ON ra.contextid = ctx.id 
        JOIN {course} co ON ctx.instanceid = co.id AND contextlevel = :contextlevel2 WHERE co.id = mc.id AND ro.id = 41) as auditor 
 ,(SELECT COUNT(cm.id) FROM {course_modules} cm join m_modules mo on mo.id = cm.module and mo.name = 'lcms' WHERE cm.course = mc.id) as lcmscount 
 ,(SELECT COUNT(cm.id) FROM {course_modules} cm  join m_modules mo on mo.id = cm.module and mo.name != 'lcms' WHERE cm.course = mc.id) as modcount 
  ,(SELECT COUNT(cm.id) FROM {course_modules} cm  join m_modules mo on mo.id = cm.module and mo.name = 'quiz' WHERE cm.course = mc.id) as quizcount 
 ,(SELECT COUNT(cm.id) FROM {course_modules} cm  join m_modules mo on mo.id = cm.module and mo.name = 'assign' WHERE cm.course = mc.id) as assigncount 
 ,(SELECT COUNT(cm.id) FROM {course_modules} cm  join m_modules mo on mo.id = cm.module and mo.name = 'workshop' WHERE cm.course = mc.id) as workshopcount
...
$sections_sql = "select DISTINCT cs.id,cs.name,cs.section , 
	...
	JOIN {modules} mo ON mo.id = cm.module AND mo.name != 'lcms' 
...
if (!$excell) {
    ...
    ?>
    <?php
    $url = "course_history.php?id=$id&excell=1&search=$search";
    ...
    <div id="content">
        <?php foreach ($courses_info as $course) { ?>
            <table cellpadding="0" cellspacing="0" class="detail"> 
                    <tr>
                        <td scope="row" class="field_title"><?php echo get_string('stats_years', 'local_lmsdata'); ?></td>
                        <td scope="col" class="field_value" style="width: 30%;"><?php echo $course->year ?></td>
                        <td scope="row" class="field_title"><?php echo get_string('stats_terms', 'local_lmsdata'); ?></td>
                        <td scope="col" class="field_value" style="width: 30%;"><?php echo $course->term ?></td>
                    </tr>
                    <tr>
                        <td scope="row" class="field_title"><?php echo get_string('stats_classification', 'local_lmsdata'); ?></td>
                        <td scope="col" class="field_value" style="width: 30%;"><?php echo $course->category_name ?></td>
                        <td scope="row" class="field_title"><?php echo get_string('stats_coursename', 'local_lmsdata'); ?></td>
                        <td scope="col" class="field_value" style="width: 30%;"><?php echo $course->fullname ?></td>
                    </tr>
                    <tr>
                        <td scope="row" class="field_title"><?php echo get_string('stats_learningperiod', 'local_lmsdata'); ?></td>
                        <td scope="col" class="field_value" style="width: 30%;"><?php echo date("Y-m-d", $course->timestart) . ' ~ ' . date("Y-m-d", $course->timeend) ?></td>
                        <td scope="row" class="field_title"><?php echo get_string('stats_student', 'local_lmsdata'); ?></td>
                        <td scope="col" class="field_value" style="width: 30%;"><?php echo $course->student;?></td>
                    </tr>
            </table>
<?php } ?>
        <table class="generaltable">
            <caption class="hidden-caption">학습이력</caption>
            <thead>
                <tr>
                    <th scope="row"><?php echo get_string('stats_student', 'local_lmsdata'); ?></th>
                    <th scope="row"><?php echo get_string('stats_weeklytopics', 'local_lmsdata'); ?></th>
                    <th scope="row"><?php echo get_string('stats_coursematerial', 'local_lmsdata'); ?></th>
                    <th scope="row"><?php echo get_string('stats_quiz', 'local_lmsdata'); ?></th>
                    <th scope="row"><?php echo get_string('stats_assignment', 'local_lmsdata'); ?></th>
                    <th scope="row"><?php echo get_string('stats_evaluation', 'local_lmsdata'); ?></th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td><?php echo $course_data->student; ?></td>                    
                    <td><?php echo $section_cnt; ?></td>
                    <td><?php echo $course_data->lcmscount; ?></td>
                    <td><?php echo $course_data->quizcount; ?></td>
                    <td><?php echo $course_data->assigncount; ?></td>
                    <td><?php echo $course_data->workshopcount; ?></td>
                </tr>
            </tbody>
        </table>  
        <form id="course_search" class="table-search-option stat_form">
            ...
            <div class="select_colume"> 
                ...
                | <span style="font-weight: bold;  cursor: pointer;" onclick="$('input[name=type]').val('write'); $('form.stat_form').submit();"><?php echo get_string('write', 'coursereport_statistics'); ?></span>
            ...
            <div class="stat_search_area">
                <input type="text" name="search" value="<?php echo $search; ?>" class="search-text" title="search" placeholder="<?php echo get_string('searchplaceholder', 'coursereport_statistics'); ?>">
    ...
    echo '<table class="generaltable stat_table"><caption class="hidden-caption">학습이력</caption><thead>'
    . '<tr>';
    if (has_capability('coursereport/statistics:isteacher', $context)) {
...
//        print_object($course_data->id);
//        print_object($course_section);
//        echo '<th colspan="' . $course_section->colspan . '">' . $course_section->name . '</th>';
        ...
        foreach ($heads as $cm) {
            ...
            if ($type == 'all' || $type == 'write') {
                $row2 .= '<span active="off" class="stat_span write_span" onclick="list_active(this,' . "'" . 'write_span' . "'" . ')">' . get_string('write', 'coursereport_statistics') . '</span>';
            }
        ...
        foreach ($users as $user) {
            ...
            foreach ($course_sections as $course_section) {
                ...
                foreach ($heads as $cm) {
                    ...
                    if ($cm->name == 'jinotechboard') {
                        if ($type == 'all' || $type == 'comment') {
                            $row3 .= '<span class="stat_span comment_span_col"> / -</span>';
                        }
                    }
                    $row3 .= '</td>';
                }
