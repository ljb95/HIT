...
join {context} ctx on ctx.contextlevel = :contextlevel5 and ctx.instanceid = mc.id 
join {role_assignments} ra on ra.contextid = ctx.id 
JOIN {role} r on r.id = ra.roleid and r.shortname = 'editingteacher' 
join {user} u on u.id = ra.userid AND u.deleted = 0
JOIN {lmsdata_user} yu ON yu.userid = u.id ";
...
<div id="contents">
    ...
    <div id="content">
<!--        <h3 class="page_title"><?php echo get_string('stats_profcourselist', 'local_lmsdata'); ?></h3>-->
        <h3 class="page_title">
            <?php if($coursetype == 0){
        ...
        <div class="page_navbar"><a href="./contact_stats_day.php"><?php echo get_string('stats_management', 'local_lmsdata'); ?></a> > <a href="<?php echo $CFG->wwwroot; ?>/siteadmin/stats/course_teacher_all.php"><?php echo get_string('stats_profcourselist', 'local_lmsdata'); ?></a> >
            <?php if($coursetype == 0){
                echo get_string('regular_course', 'local_lmsdata'); 
            }else if($coursetype == 1){
                echo get_string('irregular_course', 'local_lmsdata');
        ...
        <p class="page_sub_title"> <?php echo get_string('stats_longtext6', 'local_lmsdata'); ?></p>
        
        <div class="down_area">
             <input type="submit" onclick="course_teacher_all_excel(); return false;" class="red_btn" value="<?php echo get_string('stats_longtext4', 'local_lmsdata'); ?> (*.xls)" style="float:left;"/>
             <p style="float:left; margin: 4px 0 0 10px;"><?php echo get_string('stats_longtext2', 'local_lmsdata'); ?></p>
        </div> <!--Down Area End-->
        
        <form name="" id="course_search" class="search_area" action="course_teacher_all.php" method="get">
...
//                }
            ...
            <label><?php if($coursetype==0){?><input type="radio" title="code" name="search" value="1"<?php if($search == 1) {?> checked<?php } ?>/><?php echo get_string('course_code', 'local_lmsdata'); ?><?php }?></label>
            ...
            <input type="text" title="search" name="searchtext" value="<?php echo $searchtext; ?>" placeholder=" <?php echo get_string('stats_search', 'local_lmsdata'); ?> " class="w_260" style="color: #8E9094; margin:0 0 5px 15px;" />
        ...
        <table>
            ...
            <thead>
            <tr>
                <th scope="row" rowspan = "2"><?php echo get_string('number', 'local_lmsdata'); ?></th>
                <th scope="row" rowspan = "2"><?php echo get_string('stats_alternation', 'local_lmsdata'); ?></th>
                <th scope="row" rowspan = "2"><?php echo get_string('teachername', 'local_lmsdata'); ?></th>
                <?php if($coursetype==0){?>
                <th scope="row" rowspan = "2"><?php echo get_string('stats_years', 'local_lmsdata'); ?></th>
                <th scope="row" rowspan = "2"><?php echo get_string('stats_terms', 'local_lmsdata'); ?></th>
                <?php }?>
                <th scope="row" rowspan = "2"><?php echo get_string('stats_classification', 'local_lmsdata'); ?></th>
                <?php if($coursetype==0){?>
                <th scope="row" rowspan = "2"><?php echo get_string('course_code', 'local_lmsdata'); ?></th>
                <?php }?>
                <th scope="row" rowspan = "2"><?php echo get_string('course_name', 'local_lmsdata'); ?></th>
                <th scope="row" rowspan = "2"><?php echo get_string('teacher', 'local_lmsdata'); ?></th>
                <th scope="row" rowspan = "2"><?php echo get_string('stats_assistant', 'local_lmsdata'); ?></th>
                <th scope="row" rowspan = "2"><?php echo get_string('stats_student', 'local_lmsdata'); ?></th>
                <?php if($coursetype==0){?>
                <th scope="row" rowspan = "2"><?php echo get_string('stats_auditor', 'local_lmsdata'); ?></th>
                <?php }?>
                <th scope="row" rowspan = "2"><?php echo get_string('stats_learningactivity', 'local_lmsdata'); ?></th> 
                <th scope="row" rowspan = "2"><?php echo get_string('stats_electronicboard', 'local_lmsdata'); ?></th>
                <th scope="row" rowspan = "2"><?php echo get_string('stats_assignment', 'local_lmsdata'); ?></th>
                <th scope="row" colspan = "3" ><?php echo get_string('stats_boardread', 'local_lmsdata'); ?><br></th>
                <th scope="row" colspan = "3" ><?php echo get_string('stats_boardwrite', 'local_lmsdata'); ?><br></th>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td><?php echo $course->year; ?></td>
                        ...
                        ?>
                         <td><?php echo ($course->term == 0)? get_string('term:all','local_okregular'):$term;  ?></td>
                <?php }?>
...
<script type="text/javascript">
    function course_edit_popup(id) {
        ...
        function course_teacher_all_excel() {
        <?php
        $query_string = '';
        if(!empty($param)) {
            $query_array = array();
        ...
        ?>
        var url = "course_teacher_all_excel.php<?php echo $query_string; ?>";
