...
if($activity){
$ajoin = " 
    join( select qam.userid,a.id from  {quiz} a 
                        join {quiz_attempts} qam on qam.quiz = a.id where a.id = :id ) a on a.userid = u.id";
                        $params['id'] = $activity;
                        $usersearch .= $ajoin;
}
//if($unreport && !$activity){
    $ujoin = " 
    join (select count(id) as cnt ,course  from {quiz} where course = :cid2) ascnt on ascnt.course = c.id   
    join (select qam.userid,count(qam.id) as cnt , a.course  from {quiz} a 
                        join {quiz_attempts} qam on qam.quiz = a.id and a.course = :cid and userid != 0 
...
for ($i = 1; $i <= $section_cnt->value; $i++) {
     $query = "select a.id,cm.course ,a.name,cs.section from {course_modules} cm 
			...
			where  m.name = 'quiz' and cs.section = :section and cs.course = :courseid;";
    ...
    foreach ($section_activities as $section_activity) {
        $activities[$i][$section_activity->id] = 'quiz'; // 생성된 뷰테이블의 액티비티명  tbl , lcms , off (오프라인출석부) 정보
...
if (has_capability('moodle/course:update', $context)) {
    ...
    <!--        <div class="title"><?php echo get_string('search', 'local_attendance'); ?></div>-->
			...
			join {modules} m on m.id = cm.module 
			join {quiz} a on a.id = cm.instance 
                        join {quiz_attempts} qam on qam.quiz = a.id 
			where  m.name = 'quiz' and cs.section = :section and cs.course = :courseid;";
                for ($i = 1; $i <= $section_cnt->value; $i++) {
                ...
                <tr>
                    ...
                    <?php
                          $grade_query = "select gg.* from {grade_items} gi " 
                            ...
                            . "where itemtype = 'mod' and itemmodule = 'quiz' and iteminstance = :actid and gg.userid = :userid ";
                    for ($i = 1; $i <= $section_cnt->value; $i++) {
                        if (date('Ymd', $course->startdate + (1 * 60 * 60 * 24 * 7 * ($i))) > date('Ymd')) {
                            break;
                        }
                        ...
                        foreach ($activities[$i] as $activityid => $activity) {
                            ...
                            if($activityid != 0){
                                $att = 'X';
                                if($act = $DB->get_record('quiz_attempts',array('quiz'=>$activityid,'userid'=>$user->id))){
