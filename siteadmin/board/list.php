...
$PAGE->set_url('/siteadmin/board/list.php');

$sql = ' SELECT * FROM {jinoboard}';

$params = array();
if (!empty($search)) {
    $like = array();
    $like[] = $DB->sql_like('name', ':search1', false);
    $like[] = $DB->sql_like('engname', ':search2', false);
    $params['search1'] = '%'.$search.'%';
    $params['search2'] = '%'.$search.'%';
    
    $sql .= ' WHERE '.implode(' OR ',$like);
}
...
<div id="contents">
    ...
    <div id="content">
        ...
        <div class="page_navbar"> <a href="<?php echo $CFG->wwwroot . '/siteadmin/board/list.php'; ?>"><?php echo get_string('siteadmin_boardmanagement', 'local_lmsdata'); ?></a> > <?php echo get_string('siteadmin_boardmanagement', 'local_lmsdata'); ?></div>
        ...
        <table cellspacing="0" cellpadding="0">
            <caption class="hidden-caption">게시판 리스트</caption>
            <tr>
                <th scope="row" style="width:5%;"><?php echo get_string('number', 'local_lmsdata'); ?></th>
                <th scope="row"><?php echo get_string('name_koen', 'local_lmsdata'); ?></th>
                <th scope="row" style="width:10%;"><?php echo get_string('update_date', 'local_lmsdata'); ?></th>
                <th scope="row" style="width:10%;"><?php echo get_string('update_user', 'local_lmsdata'); ?></th>
                <th scope="row" style="width:10%;"><?php echo get_string('contents_count', 'local_lmsdata'); ?></th>
                <th scope="row" style="width:10%;"><?php echo get_string('siteadmin_act', 'local_lmsdata'); ?></th>
                <th scope="row" style="width:10%;"><?php echo get_string('siteadmin_option', 'local_lmsdata'); ?></th>
                <th scope="row" style="width:10%;"><?php echo get_string('siteadmin_management', 'local_lmsdata'); ?></th>
            </tr>
            <?php
            ...
            foreach ($datas as $data) {
                ...
                <tr>
                    ...
                    <td><?php  if($data->required == '1'){ echo get_string('siteadmin_necessary', 'local_lmsdata'); } else { ?><a href="#" onclick="if(confirm('<?php echo get_string('delete_confirm', 'local_lmsdata');?>')){ location.href='delete.php?id=<?php echo $data->id; ?>';  }"><?php echo get_string('delete', 'local_lmsdata'); ?></a><?php } ?></td>
                </tr>
                <?php
            }
            if (!$datas) {
