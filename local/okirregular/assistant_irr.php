...
$PAGE->set_url('/local/okirregular/assistant_irr.php', $params);
...
for ($i = 1; $i <= 2; $i++) {
    ...
    echo html_writer::tag('option', $i . get_string('term', 'local_lmsdata'), $params);
}
?>
                    </select>
                    <br>
                        <?php
                        //echo html_writer::tag('h6', get_string('coursename','theme_oklasscampus'), array('class'=>'courseselecter'));
...
$query = 'select c.id,c.fullname,lc.year,lc.term, u.firstname , u.lastname , lc.ohakkwa from {course} c '
        . 'join {lmsdata_class} lc on lc.isnonformal = 1 and lc.course = c.id ' . $on
