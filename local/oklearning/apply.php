...
$PAGE->set_url('/local/oklearning/apply.php');

$strplural = get_string("allcourses", "local_oklearning");
...
$sql_select = "SELECT mc.id
     ...
     , lc.timestart 
     , lc.timeend
     , lc.prof_userid, lc.isopened
     , ca.name AS category_name 
     , ca.path AS category_path 
     , u.firstname, u.lastname, lu.eng_name
     , en.enrol, en.status as enrolstatus, en.password
...
JOIN {lmsdata_class} lc ON lc.course = mc.id AND lc.isopened = 1 
JOIN {course_categories} ca ON ca.id = mc.category
JOIN {enrol} en ON en.courseid = mc.id AND en.enrol = 'self'
...
//tab
$row = array();
$row[] = new tabobject('1', "$CFG->wwwroot/local/oklearning/apply.php?type=1", get_string('ongoing', 'local_oklearning'));
$row[] = new tabobject('2', "$CFG->wwwroot/local/oklearning/apply.php?type=2", get_string('finish', 'local_oklearning'));
$rows[] = $row;
...
<form class="table-search-option" id="frm_course">
    <input type="text" title="search" name="searchval" placeholder="<?php echo get_string('search:coursename', 'local_oklearning'); ?>" value="<?php echo $searchtext; ?>"/>
...
foreach ($nums as $num) {
    ...
    echo '<option value="' . $num . '"' . $selected . '>' . get_string('showperpage', 'local_oklearning', $num) . '</option>';
}
?>
            </select>
        </div>
...
<table class="generaltable" id="table_courses">
    <caption class="hidden-caption">수강신청</caption>
    <thead>
        <tr>
            <th scope="row" width="5%"><?php echo get_string('no', 'local_oklearning'); ?></th>
            ...
            <?php if($type==1){?>
            <th scope="row" width="10%"><?php echo get_string('apply:status', 'local_oklearning'); ?></th>
            <th scope="row" width="15%"><?php echo get_string('course:apply', 'local_oklearning'); ?></th>
            <?php }?>
        </tr>
    </thead>
    <tbody>
        ...
        if ($totalcount > 0) {
            ...
            foreach ($courses as $course) {
                ...
                if ($course->enroled == 0) {
                    if($course->enrolstatus == 1){
                        $regedstatustext = get_string('apply:possible','local_oklearning');
                        $regedbutton1 = '<button onclick="apply_enrol(' . $course->id . ','.$course->enrolstatus.');" class="blue_btn_small">' . get_string('apply', 'local_oklearning') . '</button>';
                        $regedbutton = '<button onclick="hide(' . $course->id . ');" class="blue_btn_small">' . get_string('apply', 'local_oklearning') . '</button>';
                    }else {
                        $regedstatustext = get_string('apply:possible','local_oklearning');
                        $regedbutton = '<button onclick="apply_enrol(' . $course->id . ','.$course->enrolstatus.');" class="blue_btn_small">' . get_string('apply', 'local_oklearning') . '</button>';
                    }
                } else {
                    if ($course->userenrolstatus == 0) {
                        $regedstatustext = get_string('course:registered', 'local_oklearning');
                        $regedbutton = '<button class="gray_btn_small">'.get_string('course:registered','local_oklearning').'</button>';
                    } else if ($course->userenrolstatus == 1) {
                        $regedstatustext = get_string('course:approval', 'local_oklearning');
                        $regedbutton = '<button class="gray_btn_small" onclick="apply_enrol_cancel(' . $course->id . ');">'.get_string('cancel','local_oklearning').'</button>';
                    }
                }
                ?>
                <tr>
                    ...
                    <a href="courseinfo.php?id=<?php echo $course->id?>"><?php if($lang == 'ko') echo $course->course_name; else echo $course->course_name_eng; ?></a>
                    </td>
                    <td scope="col">
                        <?php
                    ...
                    <td scope="col">
                        ...
                        if (!empty($course->prof_userid)) {
                            $prof_name = ($lang=='ko')? fullname($course):$course->eng_name;
                    ...
                    <td scope="col" id="reg_button_<?php echo $course->id?>"><a id="abcd<?php echo $course->id?>"><?php echo $regedbutton;?></a><a id="abcdef<?php echo $course->id?>" style="display:none">
                        <?php echo '<input type="password" title="password" name="coursepw'.$course->id.'" id ="coursepw'.$course->id.'" value="" style = "width:70px;" /> ' .$regedbutton1;?></a></td>
                    <?php }?>
                </tr>
                <?php
            }
            ...
            $colspan = $type==1? '6':'4';
            echo '<tr><td colspan="'.$colspan.'">' . get_string('course:empty', 'local_oklearning') . '</td></tr>';
        }
        ?>
    </tbody>
</table>
...
<script type="text/javascript">
    ...
    function apply_enrol(id, status){
        var pass = $("input[name='coursepw"+id+"']").val();
        ...
        jQuery.ajax({
            url: "<?php echo $CFG->wwwroot.'/local/oklearning/apply_enrol.ajax.php'; ?>",
            data: {
                    id: id,
                    type: 1,
                    status: status,
...
//                    location.reload();
                    ...
                    if(status == 1){
                        $("td#reg_status_"+id).html("<?php echo get_string('course:registered', 'local_okirregular'); ?>");
                        $("td#reg_button_"+id).html("<button class='gray_btn_small'><?php echo ''.get_string('course:registered','local_okirregular');?></button>");
                    }else{
    ...
    function apply_enrol_cancel(id){
        ...
        if(!confirm('<?php echo get_string('apply:doyouwantcancel', 'local_oklearning'); ?>')) {
            return false;
        }
...
//                    $(this).attr("checked", false);
//                    $(this).attr("disabled", true);                        
//                    $("td#reg_status_"+id).html("<?php echo get_string('apply:possible', 'local_oklearning'); ?>");
//                    $("td#reg_button_"+id).html("<button class='blue_btn_small' onclick='apply_enrol("+id+");'><?php echo ''.get_string('apply','local_oklearning');?></button>");
                }else{
                    alert(data.message);
                }
            },
	...
	jQuery.ajax({
		url: "<?php echo $CFG->wwwroot.'/local/oklearning/parent.ajax.php'; ?>",
		data: {
			"parent" : parent
		},
		dataType: "json",
