 ...
 *
 */
function local_okregular_get_all_courses($categoryid, $param, $page = 1, $perpage = 10, $sort = " c.fullname ASC") {
    ...
    if ($page == 0) {
        ...
        $courses = $DB->get_records_sql($sql . $sort, $params, local_okregular_offset($page, $perpage), $perpage);
    }
...
function local_okregular_category($categoryid) {
    global $DB;
 ...
 * @return int
 */
function local_okregular_offset($page, $limit) {
    return $limit * ($page - 1);
}

function local_okregular_print_paging_navbar($totalcount, $page_name, $page, $perpage, $baseurl, $params = null, $maxdisplay = 10) {
    $pagelinks = array();
...
function local_okregular_print_paging_navbar_script($totalcount, $page, $perpage, $baseurl, $maxdisplay = 10) {
    $pagelinks = array();
...
function local_okregular_get_terms() {
    return array(
        '10' => get_string('term', 'local_okregular', 1),
        '11' => get_string('summer', 'local_okregular'),
        '20' => get_string('term', 'local_okregular', 2),
        '21' => get_string('winter', 'local_okregular'),
    );
}

function local_okregular_search_user_name($searchstring){
    ...
    $conditionparam['firstname'] = '%'.$searchstring.'%';
    $conditionparam['lastname'] = '%'.$searchstring.'%';
    $conditionparam['fullname'] = '%'.$searchstring.'%';
    $conditionparam['fullname1'] = '%'.$searchstring.'%';
    $conditionparam['fullname2'] = '%'.$searchstring.'%';
    $conditionparam['fullname3'] = '%'.$searchstring.'%';
    $conditionparam['username'] = '%'.$searchstring.'%';
    
    $condition->where = '('.implode(' OR ', $conditionname).')';
    $condition->params = $conditionparam;
    
    return $condition;
    
}

function local_okregular_set_assign_user($courseid, $userid, $rolename){
    global $CFG, $PAGE, $DB;
...
function local_okregular_set_unassign_user($courseid, $userid) {
