...
for ($i = 2; $i <= $maxRow; $i++) {
    ...
    if ($categoryname) {
        ...
        if (!$question = $DB->get_record('question', array('name' => $data->name, 'category' => $data->category))) {
            ...
            //multichoice 옵션 등록
            $option = $DB->get_record('qtype_multichoice_options', array('questionid'=>$question->id));
            $option->correctfeedback = $data->generalfeedback;
            $option->partiallycorrectfeedback = $data->generalfeedback;
            $option->incorrectfeedback = $data->generalfeedback;
            $DB->update_record('qtype_multichoice_options', $option);
            echo '<p>' . $i . '행 : [' . $categoryname . '] ' . $data->name . ' 옵션 정보를 업데이트하였습니다.</p>';
        }
    }else{
