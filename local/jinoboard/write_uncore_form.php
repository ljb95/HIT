...
class jinoboard_write_form extends moodleform {
    ...
    function definition() {
        ...
        $titlearray = array();
        $titlearray[] = & $mform->createElement('text', 'name', get_string('title', 'local_jinoboard'), array('size'=>'64'));
        ...
        if ($board->allowsecret) {
            ...
            if(empty($content->issecret)) {
                $mform->setDefault('issecret', 1);
            } else {
        ...
        if ($board->allowcategory) {
            $categorys = $DB->get_records('jinoboard_category', array('isused' => 1,'board'=>$bid));
            $categoryary = array();
            $categoryary[0] = get_string('emptycategory','local_jinoboard');
        ...
        $mform->addElement('hidden', 'board');
        $mform->setType('board', PARAM_INT);
        $mform->setDefault('board', $bid);
