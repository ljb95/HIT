...
$coursetype = optional_param('coursetype', 1, PARAM_INT);                 //정규(0) 비정규(1)
...
$certificate = optional_param('certificate', 0, PARAM_INT);                 //이수증 발급여부 예(1) 아니오(0)
...
$course->maxbytes = 0;
$course->enablecompletion = $certificate;
...
<script>
    alert('강의가 등록되었습니다.');
    location.href = '<?php echo $CFG->wwwroot . '/local/courselist/course_manage.php?coursetype='.$coursetype; ?>';
</script>
<?php
//    redirect($CFG->wwwroot . '/local/courselist/course_manage.php?coursetype='.$coursetype);
?>
