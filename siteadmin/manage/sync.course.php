...
<div class="content_navigation">
    <?php
        foreach($tabs AS $i=>$t) {
            ...
            echo '<a href="sync.php?tab=1&tab2='.$i.'"><p class="'.$css_class.'">'.$t['text'].'</p></a>';
        }
    ?>
</div>
<?php 
...
//                break;
    ...
    $sql_orderby = ' ORDER BY hc.hakno';

    $classes = $DB->get_records_sql('SELECT hc.id
     , hc.ohakkwa
     , hc.kor_lec_name
     , hc.prof_name
     , hc.hakno
FROM {haksa_class} hc' . $sql_where . $sql_orderby, $params, ($currpage - 1) * $perpage, $perpage);
    ...
    ?>
    <h4 class="page_sub_title">강의 동기화 이력</h4>
    <table>
        <thead>
            <th><?php echo get_string('number', 'local_lmsdata'); ?></th>
            <th>년도</th>
...
<?php
        foreach($participant_lists as $participant_list){
            ...
            echo html_writer::end_tag('td');
            echo html_writer::start_tag('td');
                echo $participant_list->timesynccourse ? date('Y-m-d H:i:s',$participant_list->timesynccourse) : '-';
            echo html_writer::end_tag('td');
