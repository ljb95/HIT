...
$params['sec2'] = $params['sec'];
if($activity){
$ajoin = " 
   left join {assign} a on a.teamsubmission = 0 and  a.id = :id 
   left join {assign_submission} search_am on search_am.assignment = a.id and search_am.userid = u.id ";
                        $params['id'] = $activity;
                        $usersearch .= $ajoin;
}
//if($unreport && !$activity){
    $ujoin = " 
    join (select count(a.id) as cnt ,a.course  from {assign} a 
        join {course_modules} cm on cm.instance = a.id 
        join {modules} m on m.name = 'assign' and m.id = cm.module 
        join {course_sections} cs on cs.id = cm.section and cs.section <= :sec2 and cs.section != 0 
        where a.course = :cid2 and a.teamsubmission = 0) ascnt on ascnt.course = c.id   
        join (select asm.userid,count(asm.id) as cnt , a.course  from {assign} a 
                        join {course_modules} cm on cm.instance = a.id 
                        join {modules} m on m.name = 'assign' and m.id = cm.module 
                        join {course_sections} cs on cs.id = cm.section and cs.section <= :sec and cs.section != 0
                        join {assign_submission} asm on asm.assignment = a.id and a.course = :cid and userid != 0 
                        where a.teamsubmission = 0   
...
for ($i = 1; $i <= $section_cnt->value; $i++) {
     $query = "select a.id,cm.course ,a.name,cs.section from {course_modules} cm 
			...
			join {assign} a on a.id = cm.instance and a.teamsubmission = 0  
			where  m.name = 'assign' and cs.section = :section and cs.course = :courseid;";
    ...
    foreach ($section_activities as $section_activity) {
        $activities[$i][$section_activity->id] = 'assign'; // 생성된 뷰테이블의 액티비티명  tbl , lcms , off (오프라인출석부) 정보
...
if (has_capability('moodle/course:update', $context)) {
    ...
    <!--        <div class="title"><?php echo get_string('search', 'local_attendance'); ?></div>-->
            <input type="number" name="grade" value="<?php echo $grade; ?>" size="5"> 점 이하 
            <br>
			...
			where  m.name = 'assign' and cs.section = :section and cs.course = :courseid;";
                for ($i = 1; $i <= $section_cnt->value; $i++) {
                    $assigns = $DB->get_records_sql($query, array('section' => $i, 'courseid' => $id));
                    echo '<optgroup label="' . $i . '주차">';
                    foreach ($assigns as $assign) {
                        echo '<option '; 
                        if($assign->id == $activity){
                            echo 'selected';
                        }
                        echo ' value="' . $assign->id . '">' . $assign->name . '</option>';
                    }
                    if (!$assigns) {
                        echo '<option disabled>과제가 없습니다.</option>';
                    }
                }
                ?>
            </select>
        ...
        <tbody>
            ...
            foreach ($users as $user) {
                ...
                <tr>
                    ...
                    <?php
                    $grade_query = "select gg.* from {grade_items} gi " 
                            . "join m_grade_grades gg on gg.itemid = gi.id "
                            . "where itemtype = 'mod' and itemmodule = 'assign' and iteminstance = :actid and gg.userid = :userid ";
                    for ($i = 1; $i <= $section_cnt->value; $i++) {
                        if (date('Ymd', $course->startdate + (1 * 60 * 60 * 24 * 7 * ($i))) > date('Ymd')) {
                            break;
                        }
                        ...
                        foreach ($activities[$i] as $activityid => $activity) {
                            ...
                            if($activityid != 0){
                                $att = 'X';
                                $class= '';
                                if($act = $DB->get_record('assign_submission',array('assignment'=>$activityid,'userid'=>$user->id))){
                                    $ugrade = $DB->get_record_sql($grade_query,array('actid'=>$activityid,'userid'=>$user->id));
                                    $grade_per = ($ugrade->finalgrade / $ugrade->rawgrademax) * 100;
                                    $att = 'O';
                                    if($ugrade->finalgrade){
                                        $att .= '<br>('.round($ugrade->finalgrade).')';
                                    }
                                    if($ugrade->finalgrade && $grade && ($grade > $ugrade->finalgrade)){
                                        $class = 'grade_row';
                                    }
                                }
                            } 
                            
                            echo '<td class="'.$class.'">' . $att .'</td>';
                        }
                    }
                    ?>
                </tr>
