...
if ($file->filepath == 'media') {
    $file->filedir = $file->filename;
     $img_nm = 'http://' . TRANS_THUMB . '/' . trim($file->filename) . '.png'; //이미지경로
} else {
    ...
    <body style="margin:0;">
        ...
        <script type="text/javascript">
            ...
            function flashplayer(auto, qua) {
                var ff = mf("player/vod_player_medic.swf?cid=<?php echo $cid; ?>" + qua + "&auto_play=" + auto + "&lrn_time=<?php echo $lrn_time; ?>&interval_num=<?php echo $interval_num; ?>&status_num=<?php echo $status_num; ?>&img_nm=<?php echo $img_nm; ?>&param1=<?php echo $param1; ?>&param2=<?php echo $param2; ?>&param3=<?php echo $param3; ?>&return_url=<?php echo $return_url; ?>&opened=<?php echo $opened ?>&key_code=<?php echo $key_code; ?>", "player", "100%", 500, "none");
